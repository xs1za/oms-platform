...
Папка `platform/` содержит общие артефакты, которые должны перейти в repository `platform`.
...
- Kafka оставить как event log для domain/integration events.
- Не внедрять отдельную очередь команд до появления реального command/job сценария.
- Запроектировать границу так, чтобы RabbitMQ/Celery можно было добавить без изменения публичных API.

Причина: преждевременное внедрение RabbitMQ или Celery увеличит операционную сложность MVP, но не даст пользы, пока нет фоновых задач с retry/DLQ/worker scaling. При этом архитектурно мы уже отделяем события `что произошло` от команд `сделай работу`, поэтому добавить очередь позже можно эволюционно.
...
В этих случаях Kafka можно оставить для публикации итоговых событий, например `report.completed` или `notification.email_status`, но саму команду лучше выполнять через RabbitMQ/Celery или другой worker mechanism.
...
| Celery + RabbitMQ | Быстрый путь для Python background jobs; готовые retry, countdown, scheduling patterns; привычная модель tasks/workers | Celery добавляет свой runtime и discipline: идемпотентность задач, сериализация, версии task signatures; нужен отдельный worker deployment | Если большинство фоновых задач Python-based и важно быстро получить production-ready workers |
| Celery + Redis | Проще локально; меньше настроек для учебного старта | Redis как broker хуже подходит для надежных очередей в production; DLQ/retry semantics слабее RabbitMQ | Только для локального прототипа, не как предпочтительный production-вариант |
...
Для MVP не добавлять RabbitMQ/Celery немедленно. Вместо этого оставить в backlog архитектурную точку расширения:

- OMS3: `ReportTask` остается API-ресурсом; позже выполнение задачи можно вынести в Celery worker без изменения endpoint `/api/v1/report-tasks`.
- OMS4: endpoint `/notifications/email` может сначала выполнять отправку синхронно/упрощенно; позже команда `send_email` может уйти в RabbitMQ/Celery, а результат будет публиковаться в Kafka как `notification.email_status`.
- Platform: при появлении первой production-like фоновой команды добавить manifests для RabbitMQ и worker deployments.
...
Если бизнес-гипотеза подтвердится, наиболее практичный следующий шаг - `RabbitMQ + Celery` для Python-сервисов OMS3/OMS4 и сохранение Kafka как event log для межсервисных событий.
